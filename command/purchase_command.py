...
from command.transaction_memento import TransactionMemento
...
class PurchaseItemCommand(Command):
    ...
    def execute(self) -> dict:
        ...
        self._result = {
            ...
            "total_amount": total,
            "message": f"✅ Purchased {product['name']} x{actual_qty} for ${total:.2f}",
            "state_note": ""
        }
        return self._result
    ...
    def undo(self) -> dict:
        ...
        return {
            "success": True,
            "message": f"🔙 Rolled back: {self._result['product_name']} x{self._result['quantity']} — ${self._result['total_amount']:.2f} refunded",
